return function(require)
	...
	local Icons = {
		Assets = {
			...
			["key-round"] = "rbxassetid://7733965118",
			...
			["layers"] = "rbxassetid://93025155356613",
			...
			["palette"] = "rbxassetid://76682043305646",
			...
			["save"] = "rbxassetid://93373712935751",
			...
			["type"] = "rbxassetid://116040089432583",
	...
	function Icons:RegisterMany(values)
		for name, value in pairs(values) do
			self:Register(name, value)
		end
	end
	function Icons:Get(name)
		return self.Assets[aliases[name] or name]
	end
	function Icons:Resolve(value)
		if type(value) == "table" then
			return value.Image or value.Asset or value[1]
		end
		if type(value) == "number" then
			return "rbxassetid://" .. value
		end
		return self:Get(value)
			or (type(value) == "string" and string.find(value, "://", 1, true) and value)
			or self.Assets.circle
	end
	function Icons:Apply(label, value, options)
		options = options or {}
		label.Image = self:Resolve(value)
		label.ScaleType = Enum.ScaleType.Fit
		label.ResampleMode = Enum.ResamplerMode.Default
		if options.Color then
			label.ImageColor3 = options.Color
		end
		if options.Size then
			label.Size = typeof(options.Size) == "UDim2" and options.Size
				or UDim2.fromOffset(options.Size, options.Size)
		end
		return label
	end
	function Icons.create(window, parent, name, token, size)
		local label = Core.new(
			"ImageLabel",
			{ Name = name or "Icon", BackgroundTransparency = 1, Size = UDim2.fromOffset(size or 16, size or 16) },
			parent
		)
		Icons:Apply(label, name or "circle")
		Core.bind(window, label, "ImageColor3", token or "TextSecondary")
		return label
	end
	return Icons
end
